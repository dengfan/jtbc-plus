...
public partial class module: jpage
{
  ...
  private string Module_List()
  {
    ...
    if (com.fileExists(Server.MapPath(trootstr)))
    {
      ...
      try
      {
        ...
        for (int ti = 0; ti < tXMLNodesCount; ti ++)
        {
          ...
          {
            tmptstr = tmpastr;
            string tValueTpl = jt.itake("manage-interface.data-value-textarea", "tpl");
            if (tis == 0) tValueTpl = jt.itake("manage-interface.data-value-input", "tpl");
      ...
      if (!cls.isEmpty(tfield))
      {
        ...
        for (int tis = 0; tis < tfieldArys.Length; tis ++)
        {
          tmptstr = tmpastr;
          string tValueTpl = jt.itake("manage-interface.data-value-textarea", "tpl");
          if (tis == 0) tValueTpl = jt.itake("manage-interface.data-value-input-2", "tpl");
    ...
    {
      tmpstr = jt.itake("manage-interface.list-error", "tpl");
      tmpstr = jt.creplace(tmpstr);
    }
    tmpstr = config.ajaxPreContent + tmpstr;
    return tmpstr;
