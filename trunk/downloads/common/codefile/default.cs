...
public partial class module: jpage
{
  private string PP_FormatURLs(string argurls)
  {
    ...
    if (!cls.isEmpty(turls))
    {
      ...
      if (turlsAry != null)
      {
        string tLinkStr = jt.itake("default.data_download", "tpl");
        for (int ti = 0; ti < turlsAry.Length; ti ++)
        {
          string turlsAryStr = turlsAry[ti];
          if (!cls.isEmpty(turlsAryStr))
          {
            string[] turlsAryStrAry = cls.split(turlsAryStr, "|+|");
            if (turlsAry != null)
            {
  ...
  private string Module_Detail()
  {
    ...
    if (tArys != null)
    {
      ...
      tmpstr = tmpstr.Replace("{$-genre}", config.ngenre);
      tmpstr = tmpstr.Replace("{$-lng}", cls.toString(config.nlng));
      tmpstr = tmpstr.Replace("{$-download}", PP_FormatURLs((string)db.getValue(tAry, "urls")));
      tmpstr = jt.creplace(tmpstr);
    }
    return tmpstr;
  }
