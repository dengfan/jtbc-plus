            ...
            try
            {
                ...
                if (!com.directoryExists(tNewGenrePath))
                {
                    ...
                    for (int ti = 0; ti < tXMLNodes.Count; ti++)
                    {
                        ...
                        if (!(com.directoryCreate(Server.MapPath(cls.getActualRoute(cls.getLRStr(tfilepath, "/", "leftr")))) && //创建文件夹
                            com.fileCreate(Server.MapPath(cls.getActualRoute(tfilepath)), encode.base64.decodeBase64bt(tfilevalue)))) //创建文件（即解密XML文件）
                            tState1 = 1;
                    }
                    if (tState1 == 0) //全部创建成功
                    {
                        ...
                        #endregion
                        if (tState2 == 0 && tdbtype == "0") //access 数据库创建表成功
                        {
                            string[,] tconfigAry3 = jt.itakes("global." + tNewGenre + ":config.all", "cfg"); //access 版每个模块下的config.jtbc里应该都有一个all节点
                        ...
                        if (com.directoryDelete(Server.MapPath(cls.getActualRoute(tNewGenre) + "/_install"))) //删除_install目录
                        {
