...
public partial class module: jpage
{
  ...
  {
    string tmpstr = "";
    ac.cntitle(jt.itake("default.nav_manage_add", "lng"));
    tmpstr = jt.itake("default.manage-add", "tpl");
  ...
  private string Module_Manage_Detail()
  {
    ...
    if (tArys != null)
    {
      ...
      tmpstr = jt.itake("default.manage-detail", "tpl");
      for (int ti = 0; ti < tAry.GetLength(0); ti ++)
      {
        tAry[ti, 0] = (object)cls.getLRStr((string)tAry[ti, 0], tfpre, "rightr");
      ...
      config.rsAry = tAry;
      if ((int)db.getValue(tAry, "read") != 1) db.Execute("update " + tdatabase + " set " + cls.cfnames(tfpre, "read") + "=1 where " + tidfield + "=" + tId);
      tmpstr = tmpstr.Replace("{$-genre}", config.ngenre);
      tmpstr = tmpstr.Replace("{$-lng}", cls.toString(config.nlng));
      tmpstr = tmpstr.Replace("{$-defmenu}", account.getDefMenuHtml());
      tmpstr = jt.creplace(tmpstr);
  ...
  private string Module_Manage()
  {
    ...
    else
    {
      ...
      {
        case "add":
          tmpstr = Module_Manage_Add();
          break;
        case "list":
          tmpstr = Module_Manage_List();
          break;
        case "detail":
          tmpstr = Module_Manage_Detail();
          break;
        default:
          tmpstr = Module_Manage_List();
          break;
